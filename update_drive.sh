...
#Notes:
#if micron statement works with update_micron_mu05.sh! please include that!!

#Updates:
# 060914
# -Added some notes, getting ready to use for APLYH and update micron drives
# -Added if micron, if raidcard steps

# 061914
# -Adding function to check & parse list for ss-code

# 072214
# -Added check for fw file

# 080514
# -Added check for SAS/SATA type, since they use different syntax for MegaCli.

# 080614
# -Removed the dot slash before hdparm in function resize_ssd

# 080614
# -Changed Device ID to Slot Number in MegaRAID drive update

# 081314
# -Added regexp capability with use of -x flag

# 082914
# -Added the use of new seagate tool

# 100614
# -Added support for PMC Adaptec drives using arcconf utility
# -Used more functions to give the script a cleaner look

#This script is compiled and encrypted, don't steal it!
...
if [ -e $fwfile ]; then
	if [ "$tool" == 'seagate' ]; then
		...
		returnCode=$?; check_status $returnCode
	else
...
#Check For List, Parse List, Define Variables
parse_list(){
if [ ! -e $hdd_list ]; then
...
#SmartCtl is wack. SATA drive information come up different then SAS drives. making a function to fix this.
set_variables(){
if [ "$pdType" == 'SAS' ]; then
	raidParam='megaraid'
...
#ymodel/yfw is just model/fw found in raidcard, i'm too stupid and lazy to come up with better name. sorry. [apology accepted -john]
...
# Use mode 3 for SATA drive, use mode 7 for SAS drive
...
# My Super Ghetto way of making the array
...
# Debug Purposes / Uncomment to display on screen
# The output below should be the same in terms of length.

#echo "${#hd_array[@]}"
#echo "${#fw_array[@]}"
#echo "${#channel_array[@]}"
#echo "${#id_array[@]}"
#echo "${#controller_array[@]}"
}

update_onboard_drives() {
#Function to update firmware on onboard drives
#I'm using x as the current drive variable, hence xmodel and xfw
echo -e "***********************************"
...
for x in /dev/sd*[a-z]; do
	...
	# Check which brand they are
	# btw, correctDrive is now misnamed, it should really be something like driveBrand
	if [ "${regexp}" == 'yes' ]; then
	...
	# If special tool is used for specific brand, use it. Otherwise default to normal 
	if [ "${correctDrive}" == 'micron' ]; then #If Micron update all microns
	...
	elif [ "${correctDrive}" == 'default' ]; then # Normal onboard hdparm update
		if [ "${xfw}" != "${fw}" ]; then
			echo -e "Drive ${x} FW ${xfw} -> ${RD}[ Incorrect ]${RS}\n"
			echo "Preparing to flash FW..."
			update_fw
...
#call function to update onboard drives
update_onboard_drives

#Checking if any LSI MegaRAID devices exists, calls function if true
lspci | grep -i "MegaRAID" > /dev/null
if [ $? == 0 ]; then
    update_lsi_drives
fi

#Checking if any PMC Adaptec devices exists, calls function if true
